...
ipm_baqbarrio.drop(columns=['Vulnerabilidad'], inplace=True)
ipm_baqbarrio_mean=ipm_baqbarrio.groupby('BARRIO').mean()
...
## graph conteodecolegios with bar chart and ipm with points
    ...
    gb=GridOptionsBuilder.from_dataframe(barriossincol[{'BARRIO_left','distanciakm', 'ipm', 'nearcolegio'}].sort_values(by='ipm',ascending=False))
    gb.configure_pagination(paginationAutoPageSize=True)
    gb.configure_side_bar()
    gridOptions=gb.build()
    grid_response=AgGrid(barriossincol[{ 'BARRIO_left','distanciakm', 'ipm', 'nearcolegio'}].sort_values(by='ipm',ascending=False), gridOptions=gridOptions,
                        height=400,
                        width='100%',
                        data_return_mode=DataReturnMode.FILTERED_AND_SORTED,
                        fit_columns_on_grid_load=True,
